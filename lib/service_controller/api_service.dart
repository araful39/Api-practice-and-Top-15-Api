...
Future<List<Model01>> getApi01()async{
 ...
 List<dynamic> data=decode["entries"];
  ...
  List<Model01> articlesList=data.map((item)=>Model01.fromJson(item)).toList();
   return articlesList;
 }else {
   print("Data not");
...
Future<Model06> getApi06()async{
  ...
  Future<List<Model07>> getApi07()async{
    ...
    if(response.statusCode==200){
      ...
      List<Model07> articlesList07=data.map((item)=>Model07.fromJson(item)).toList();
...
Future<Model08> getApi08()async{
  String url08="https://datausa.io/api/data?drilldowns=Nation&measures=Population";
